(function (r) {
    r.fn.qrcode = function (h) {
        let s; function u(a) { this.mode = s; this.data = a; } function o(a, c) { this.typeNumber = a; this.errorCorrectLevel = c; this.modules = null; this.moduleCount = 0; this.dataCache = null; this.dataList = []; } function q(a, c) { if (void 0 == a.length) throw Error(`${a.length}/${c}`); for (var d = 0; d < a.length && a[d] == 0;)d++; this.num = Array(a.length - d + c); for (let b = 0; b < a.length - d; b++) this.num[b] = a[b + d]; } function p(a, c) { this.totalCount = a; this.dataCount = c; } function t() { this.buffer = []; this.length = 0; }u.prototype = { getLength() { return this.data.length; },
            ...
            isDark(a, c) { if (a < 0 || this.moduleCount <= a || c < 0 || this.moduleCount <= c) throw Error(`${a},${c}`); return this.modules[a][c]; },
...
d >= 0 && d <= 6 && (b == 0 || b == 6) || b >= 0 && b <= 6 && (d == 0 || d == 6) || d >= 2 && d <= 4 && b >= 2 && b <= 4 ? !0 : !1);
            ...
            getBestMaskPattern() { for (var a = 0, c = 0, d = 0; d < 8; d++) { this.makeImpl(!0, d); const b = j.getLostPoint(this); if (d == 0 || a > b)a = b, c = d; } return c; },
            createMovieClip(a, c, d) { a = a.createEmptyMovieClip(c, d); this.make(); for (c = 0; c < this.modules.length; c++) for (var d = 1 * c, b = 0; b < this.modules[c].length; b++) { const e = 1 * b; this.modules[c][b] && (a.beginFill(0, 100), a.moveTo(e, d), a.lineTo(e + 1, d), a.lineTo(e + 1, d + 1), a.lineTo(e, d + 1), a.endFill()); } return a; },
            setupTimingPattern() { for (var a = 8; a < this.moduleCount - 8; a++) this.modules[a][6] == null && (this.modules[a][6] = a % 2 == 0); for (a = 8; a < this.moduleCount - 8; a++) this.modules[6][a] == null && (this.modules[6][a] = a % 2 == 0); },
            setupPositionAdjustPattern() {
                for (let a = j.getPatternPosition(this.typeNumber), c = 0; c < a.length; c++) {
                    for (let d = 0; d < a.length; d++) {
                        let b = a[c],
                            e = a[d]; if (this.modules[b][e] == null) for (let f = -2; f <= 2; f++) for (let i = -2; i <= 2; i++) this.modules[b + f][e + i] = f == -2 || f == 2 || i == -2 || i == 2 || f == 0 && i == 0 ? !0 : !1;
                    }
                }
            },
            setupTypeNumber(a) {
...
0, b = 0, e = 0, f = Array(c.length), i = Array(c.length), g = 0; g < c.length; g++) {
                var n = c[g].dataCount,
                    h = c[g].totalCount - n,
                    b = Math.max(b, n),
                    e = Math.max(e, h); f[g] = Array(n); for (var k = 0; k < f[g].length; k++)f[g][k] = 255 & a.buffer[k + d]; d += n; k = j.getErrorCorrectPolynomial(h); n = (new q(f[g], k.getLength() - 1)).mod(k); i[g] = Array(k.getLength() - 1); for (k = 0; k < i[g].length; k++)h = k + n.getLength() - i[g].length, i[g][k] = h >= 0 ? n.get(h) : 0;
...
(d[n++] = f[g][k]);
            ...
            G18: 7973,
            G15_MASK: 21522,
            getBCHTypeInfo(a) { for (var c = a << 10; j.getBCHDigit(c) - j.getBCHDigit(j.G15) >= 0;)c ^= j.G15 << j.getBCHDigit(c) - j.getBCHDigit(j.G15); return (a << 10 | c) ^ j.G15_MASK; },
            getBCHTypeNumber(a) {
                for (var c = a << 12; j.getBCHDigit(c) -
j.getBCHDigit(j.G18) >= 0;)c ^= j.G18 << j.getBCHDigit(c) - j.getBCHDigit(j.G18); return a << 12 | c;
            },
            getBCHDigit(a) { for (var c = 0; a != 0;)c++, a >>>= 1; return c; },
            getPatternPosition(a) { return j.PATTERN_POSITION_TABLE[a - 1]; },
            getMask(a, c, d) {
                switch (a) {
            ...
            getErrorCorrectPolynomial(a) { for (var c = new q([1], 0), d = 0; d < a; d++)c = c.multiply(new q([1, l.gexp(d)], 0)); return c; },
            getLengthInBits(a, c) {
...
2) && a.isDark(b, e + 3) && a.isDark(b, e + 4) && !a.isDark(b, e + 5) && a.isDark(b, e + 6) && (d += 40);
            ...
            } }, l = { glog(a) { if (a < 1) throw Error(`glog(${a})`); return l.LOG_TABLE[a]; },
                gexp(a) { for (;a < 0;)a += 255; for (;a >= 256;)a -= 255; return l.EXP_TABLE[a]; },
                EXP_TABLE: Array(256),
                LOG_TABLE: Array(256) }, m = 0; m < 8; m++)l.EXP_TABLE[m] = 1 << m; for (m = 8; m < 256; m++)l.EXP_TABLE[m] = l.EXP_TABLE[m - 4] ^ l.EXP_TABLE[m - 5] ^ l.EXP_TABLE[m - 6] ^ l.EXP_TABLE[m - 8]; for (m = 0; m < 255; m++)l.LOG_TABLE[l.EXP_TABLE[m]] = m; q.prototype = { get(a) { return this.num[a]; },
            getLength() { return this.num.length; },
            multiply(a) { for (var c = Array(this.getLength() + a.getLength() - 1), d = 0; d < this.getLength(); d++) for (let b = 0; b < a.getLength(); b++)c[d + b] ^= l.gexp(l.glog(this.get(d)) + l.glog(a.get(b))); return new q(c, 0); },
            mod(a) {
            ...
            let a; if (h.render == 'canvas') {
                a = new o(h.typeNumber, h.correctLevel); a.addData(h.text); a.make(); var c = document.createElement('canvas'); c.width = h.width; c.height = h.height; for (var d = c.getContext('2d'), b = h.width / a.getModuleCount(), e = h.height / a.getModuleCount(), f = 0; f < a.getModuleCount(); f++) {
                    for (var i = 0; i < a.getModuleCount(); i++) {
                        d.fillStyle = a.isDark(f, i) ? h.foreground : h.background; let g = Math.ceil((i + 1) * b) - Math.floor(i * b),
                            j = Math.ceil((f + 1) * b) - Math.floor(f * b); d.fillRect(Math.round(i * b), Math.round(f * e), g, j);
                    }
                }
            } else {
                a = new o(h.typeNumber, h.correctLevel); a.addData(h.text); a.make(); c = r('<table></table>').css('width', `${h.width}px`).css('height', `${h.height}px`).css('border', '0px')
                    .css('border-collapse', 'collapse')
                    .css('background-color', h.background); d = h.width / a.getModuleCount(); b = h.height / a.getModuleCount(); for (e = 0; e < a.getModuleCount(); e++) {
